...
def mock_authorize_redirect(monkeypatch):
    async def fake_authorize_redirect(request, redirect_uri):
        return RedirectResponse(url="/somewhere")
...
def test_auth_login_google_redirect(client, mock_authorize_redirect):
    ...
    print("\n[DEBUG] login_google status:", resp.status_code)
    print("[DEBUG] login_google headers:", resp.headers)
    print("[DEBUG] login_google content:", resp.content)
...
def test_auth_callback_google_creates_and_returns_jwt(client, mock_authorize_access_token, mock_parse_id_token):
    ...
    print("\n[DEBUG] callback_google status:", response.status_code)
    print("[DEBUG] callback_google headers:", response.headers)
    print("[DEBUG] callback_google content:", response.content)
...
def test_auth_callback_google_updates_existing_user(client, mock_authorize_access_token, mock_parse_id_token_existing):
    ...
    response = client.get("/api/v1/auth/callback/google", headers={"origin": "http://localhost:3000"})
    print("\n[DEBUG] callback_google_update status:", response.status_code)
    print("[DEBUG] callback_google_update headers:", response.headers)
    print("[DEBUG] callback_google_update content:", response.content)
    assert response.status_code in (302, 307)
...
def test_auth_callback_google_oauth_error(client, mock_authorize_access_token_oauth_error):
    ...
    assert "OAuth authentication failed" in response.text or "OAuth failed" in response.text

def test_auth_callback_google_missing_userinfo(client, mock_authorize_access_token, mock_parse_id_token_none):
    response = client.get("/api/v1/auth/callback/google", headers={"origin": "http://localhost:3000"})
    assert response.status_code == 400
    assert "Failed to retrieve user info" in response.text
...
def test_auth_protected_route_with_valid_jwt(client):
    ...
    print("\n[DEBUG] protected_route status:", resp.status_code)
    print("[DEBUG] protected_route headers:", resp.headers)
    print("[DEBUG] protected_route content:", resp.content)
